...
var helpDoc = MM.HELP_behEffectPulsate;


var ELEMENTNAMES_ALLOWED; // list of elements to which the Pulsate Effect can be asigned (initialized in "initGlobals()")
...
// Adds an Pulsate-Effect to the element.
// Accepts the following arguments:
//  targetElement - ID or JavaScript DOM object of target element
//  method        - keyword "effect"
//  effect        - "pulsate"
//  mode          - The mode of the effect. Can be "show" or "hide"
//  times         - Times to pulsate
...
{
    dw.logEvent(UT_JQUERY_PULSATE_EFFECT, UT_JQUERY_PULSATE_EFFECT_APPLY);
	...
	{
	    retVal = "MM_DW_effectPulsate($(this),'effect','pulsate','" + document.theForm.modeTypeObj.options[document.theForm.modeTypeObj.selectedIndex].value + "'," +  document.theForm.timesObj.value + "," + document.theForm.speedObj.value + ")";
	}
	else // behavior element triggers effect which is assigned to a target element
	{
		var refIdx = parseInt(selValue);
		retVal = "MM_DW_effectPulsate($('#" + ID_LIST[refIdx] + "'),'effect','pulsate','" + document.theForm.modeTypeObj.options[document.theForm.modeTypeObj.selectedIndex].value + "'," + document.theForm.timesObj.value + "," + document.theForm.speedObj.value + ")";
	}
...
function inspectBehavior(fnStr){
    dw.logEvent(UT_JQUERY_PULSATE_EFFECT, UT_JQUERY_PULSATE_EFFECT_INSPECT);
    var argArray = extractExprStr(fnStr);
  if (argArray.length == 6) { // we expect 6 args -> targetElement, effect(keyword),effect(pulsate), mode, times, speed(duration)
	var selIdx  = 0;
	...
	var modeIdx = (argArray[3] == "show")?0:1;
	var timesValue = effectsUtils.stripWhitespaces(unescExprStr(argArray[4], false));
...
{
    dw.logEvent(UT_JQUERY_PULSATE_EFFECT, UT_JQUERY_PULSATE_EFFECT_INITIALIZE);
	...
	// there are potential elements to which the Pulsate Effect can be applied to
	...
	document.theForm.timesObj.value = "5"; // Times to pulsate
