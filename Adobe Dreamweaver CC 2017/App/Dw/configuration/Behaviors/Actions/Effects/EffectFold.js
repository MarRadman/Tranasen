...
var helpDoc = MM.HELP_behEffectFold;
...
// Adds an Fold-Effect to the element.
...
//  effect        - the name of the effect i.e. fold
//  horizFirst    - Whether to fold horizontally first or not. Can be true or false
//  size          - Size to be folded to(Integer)

function MM_DW_effectFold(obj,method,effect,horizFirst,size,speed)
{
    obj[method](effect, { horizFirst:horizFirst, size:size}, speed);
}
...
{
    dw.logEvent(UT_JQUERY_FOLD_EFFECT, UT_JQUERY_FOLD_EFFECT_APPLY);
	...
	{
	    retVal = "MM_DW_effectFold($(this),'" + document.theForm.effectTypeObj.options[document.theForm.effectTypeObj.selectedIndex].value + "'" + ",'fold'," + document.theForm.horizFirstTypeObj.options[document.theForm.horizFirstTypeObj.selectedIndex].value + "," + document.theForm.sizeObj.value + "," + document.theForm.speedObj.value + ")";
	}
	else // behavior element triggers effect which is assigned to a target element
	{
		var refIdx = parseInt(selValue);
		retVal = "MM_DW_effectFold($('#" + ID_LIST[refIdx] + "'),'" + document.theForm.effectTypeObj.options[document.theForm.effectTypeObj.selectedIndex].value + "'" + ",'fold'," + document.theForm.horizFirstTypeObj.options[document.theForm.horizFirstTypeObj.selectedIndex].value + "," + document.theForm.sizeObj.value + "," + document.theForm.speedObj.value + ")";
	}
...
function inspectBehavior(fnStr){
    dw.logEvent(UT_JQUERY_FOLD_EFFECT, UT_JQUERY_FADE_EFFECT_INSPECT);
    var argArray = extractExprStr(fnStr);
  if (argArray.length == 6) { // we expect 6 args -> targetElement, method, effect, horizFirst, size, speed(duration)
	var selIdx  = 0;
	...
	var horizFirstIdx = (argArray[3] == "false") ? 0 : 1;
	var sizeValue = effectsUtils.stripWhitespaces(unescExprStr(argArray[4], false));
	var speedValue = effectsUtils.stripWhitespaces(unescExprStr(argArray[5], false));
	document.theForm.effectTypeObj.selectedIndex = effectIdx;
	document.theForm.horizFirstTypeObj.selectedIndex = horizFirstIdx;
	document.theForm.sizeObj.value = parseFloat(sizeValue);
...
{
    dw.logEvent(UT_JQUERY_FOLD_EFFECT, UT_JQUERY_FADE_EFFECT_INITIALIZE);
	...
	// there are potential elements to which the Fold Effect can be applied to
	...
	document.theForm.horizFirstTypeObj.selectedIndex = selIdx;
	document.theForm.sizeObj.value = "15"; // Size to be folded to
