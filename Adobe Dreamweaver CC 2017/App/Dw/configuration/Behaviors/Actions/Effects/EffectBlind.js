...
//  effect        - the name of the effect i.e. blind
...
{
    dw.logEvent(UT_JQUERY_BLIND_EFFECT, UT_JQUERY_BLIND_EFFECT_APPLY);
	...
	{
	    retVal = "MM_DW_effectBlind($(this),'" + document.theForm.effectTypeObj.options[document.theForm.effectTypeObj.selectedIndex].value + "'" + ",'blind','" + document.theForm.directionTypeObj.options[document.theForm.directionTypeObj.selectedIndex].value + "'," + document.theForm.speedObj.value + ")";
	}
	else // behavior element triggers effect which is assigned to a target element
	{
		var refIdx = parseInt(selValue);
		retVal = "MM_DW_effectBlind($('#" + ID_LIST[refIdx] + "'),'" + document.theForm.effectTypeObj.options[document.theForm.effectTypeObj.selectedIndex].value + "'" + ",'blind','" + document.theForm.directionTypeObj.options[document.theForm.directionTypeObj.selectedIndex].value + "'," + document.theForm.speedObj.value + ")";
	}
...
function inspectBehavior(fnStr){
    dw.logEvent(UT_JQUERY_BLIND_EFFECT, UT_JQUERY_BLIND_EFFECT_INSPECT);
  ...
  if (argArray.length == 5) { // we expect 5 args -> targetElement, method, effect, direction,speed(duration)
	...
	var directionIdx = (argArray[3] == "up")?0:(argArray[3] == "down")?1:(argArray[3] == "left")?2:(argArray[3] == "right")?3:(argArray[3] == "vertical")?4:5;
...
{
    dw.logEvent(UT_JQUERY_BLIND_EFFECT, UT_JQUERY_BLIND_EFFECT_INITIALIZE);
	...
	//there are potential elements to which the Blind Up/Down Effect can be applied to
