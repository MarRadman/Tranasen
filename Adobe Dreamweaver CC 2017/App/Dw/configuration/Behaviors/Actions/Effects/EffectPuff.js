...
var helpDoc = MM.HELP_behEffectPuff;
...
// Adds an Puff-Effect to the element.
...
//  effect        - the name of the effect i.e. puff
//  percent 	  - The percentage to scale to

function MM_DW_effectPuff(obj,method,effect,percent,speed)
{
    obj[method](effect, { percent:percent}, speed);
}
...
{
    dw.logEvent(UT_JQUERY_PUFF_EFFECT, UT_JQUERY_PUFF_EFFECT_APPLY);
	...
	{
	    retVal = "MM_DW_effectPuff($(this),'" + document.theForm.effectTypeObj.options[document.theForm.effectTypeObj.selectedIndex].value + "'" + ",'puff'," + document.theForm.percentObj.value + "," + document.theForm.speedObj.value + ")";
	}
	else // behavior element triggers effect which is assigned to a target element
	{
		var refIdx = parseInt(selValue);
		retVal = "MM_DW_effectPuff($('#" + ID_LIST[refIdx] + "'),'" + document.theForm.effectTypeObj.options[document.theForm.effectTypeObj.selectedIndex].value + "'" + ",'puff'," + document.theForm.percentObj.value + "," + document.theForm.speedObj.value + ")";
	}
...
function inspectBehavior(fnStr){
    dw.logEvent(UT_JQUERY_PUFF_EFFECT, UT_JQUERY_PUFF_EFFECT_INSPECT);
    var argArray = extractExprStr(fnStr);
  if (argArray.length == 5) { // we expect 5 args -> targetElement, method, effect, percent, speed(duration)
	var selIdx  = 0;
	...
	var percentValue = effectsUtils.stripWhitespaces(unescExprStr(argArray[3], false));
	...
	document.theForm.percentObj.value = parseFloat(percentValue);
...
{
    dw.logEvent(UT_JQUERY_PUFF_EFFECT, UT_JQUERY_PUFF_EFFECT_INITIALIZE);
	...
	// there are potential elements to which the Puff Effect can be applied to
	...
	document.theForm.percentObj.value = "150"; // The percentage to scale to
