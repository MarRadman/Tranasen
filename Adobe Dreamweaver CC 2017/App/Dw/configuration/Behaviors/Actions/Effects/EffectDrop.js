...
//*************** GLOBALS VARS *****************
var helpDoc = MM.HELP_behEffectDrop;
...
// Adds an Drop-Effect to the element.
...
//  effect        - the name of the effect i.e. drop
...
{
    dw.logEvent(UT_JQUERY_DROP_EFFECT, UT_JQUERY_DROP_EFFECT_APPLY);
	...
	{
	    retVal = "MM_DW_effectDrop($(this),'" + document.theForm.effectTypeObj.options[document.theForm.effectTypeObj.selectedIndex].value + "'" + ",'drop','" + document.theForm.directionTypeObj.options[document.theForm.directionTypeObj.selectedIndex].value + "'," + document.theForm.speedObj.value + ")";
	}
	else // behavior element triggers effect which is assigned to a target element
	{
		var refIdx = parseInt(selValue);
		retVal = "MM_DW_effectDrop($('#" + ID_LIST[refIdx] + "'),'" + document.theForm.effectTypeObj.options[document.theForm.effectTypeObj.selectedIndex].value + "'" + ",'drop','" + document.theForm.directionTypeObj.options[document.theForm.directionTypeObj.selectedIndex].value + "'," + document.theForm.speedObj.value + ")";
	}
...
function inspectBehavior(fnStr){
    dw.logEvent(UT_JQUERY_DROP_EFFECT, UT_JQUERY_DROP_EFFECT_INSPECT);
...
{
    dw.logEvent(UT_JQUERY_DROP_EFFECT, UT_JQUERY_DROP_EFFECT_INITIALIZE);
	...
	// there are potential elements to which the Drop Effect can be applied to
